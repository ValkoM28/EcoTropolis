...
public class Tokyo : Location {
    ...
    // Constructor to initialize the Tokyo location with the game and player instances
    public Tokyo(Game game, Player player) : base("Tokyo") {
        _player = player;
        _game = game;
    }

    // Override the Play method to define the behavior when the player is in Tokyo
    public override void Play() {
        // Create an instance of the world map for Asia
        MapAsia worldMap = new();
        Console.Clear();
        worldMap.DisplayMap();
        DisplayStartMessage();
        ...
        while (playing) { //game loop logic here 
            ...
            {
                TokyoMessages.PrintStartMessage();
            }
        }
    }
    
    // Override the DisplayStartMessage method to show a message when the player enters Tokyo
    public override void DisplayStartMessage() {
        Console.WriteLine("\nYou are now in Tokyo. \n" +
                          "Type \"start\" to begin a new journey ");
    }

    // Override the ShowDescription method to provide a description of Tokyo
